...
> [!NOTE]
> **测试环境**：CPU 参考环境（无 GPU），使用真实微调角色模型（CyreneV3.7 / shouanren，v2ProPlus 兼容架构），短文本推理平均值。

| 指标 | 共享骨干 | 全量加载 | 说明 |
| :--- | :---: | :---: | :--- |
| 单角色推理平均延迟 | 0.70s | 0.70s | ⚖️ 无性能损失 |
| 峰值内存 (RAM) | **2.66 GB** | 3.20 GB | 💾 节省 **17%**（CPU 实测） |
| 2 角色初始化耗时 | 34.4s | 9.1s | 含一次性权重提取，后续换角色零开销 |

> [!IMPORTANT]
> **架构兼容性验证**（真实模型）：
> - ✅ CyreneV3.7、shouanren → 共享骨干模式（仅提取 25 GPT keys + 37 SoVITS keys）
> - ⚠️ aimisi（v2 架构，`upsample_initial_channel=512` vs base `768`）→ **自动降级**为完整模型加载，不影响其他角色
>
> GPU 环境下显存节省远高于 CPU 实测值（权重注入不依赖显存带宽，VRAM 对比见上方 MultiSpeaker 显存对比表）；推理延迟共享骨干与全量加载基本一致，收益集中在显存与多角色并发场景。
<br>
